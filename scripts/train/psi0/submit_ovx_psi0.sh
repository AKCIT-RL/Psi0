...
STATE_NOISE_STD_WAIST="${STATE_NOISE_STD_WAIST:-}"
...
apptainer exec --nv --cleanenv \
    ...
    bash -lc '
        set -euo pipefail
        cd /workspace
        # pick the first venv whose python actually has the training deps
        if .venv-psi/bin/python -c "import torch" 2>/dev/null; then
            source .venv-psi/bin/activate
        elif .venv/bin/python -c "import torch" 2>/dev/null; then
            source .venv/bin/activate
        else
            echo "ERROR: no venv with torch found in the SIF (/workspace/.venv-psi, /workspace/.venv)" >&2
            exit 1
        fi
